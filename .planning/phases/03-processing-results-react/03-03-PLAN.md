...
4. **Retry handlers**:
   ...
   - `handleRetryAllFailed()`: call `useRetryBatchMutation` (from batchesApi.ts, added in Plan 03-02) which calls `POST /api/v1/batches/${batchName}/retry`. This is distinct from `useStartBatchMutation` which calls `/start` and would re-process the entire batch from scratch. Set `isProcessing(true)` and navigate to processing step.
