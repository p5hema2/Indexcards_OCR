...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/frontend/src/api/batchesApi.ts"
      provides: "cancelBatch and fetchResults API functions"
      contains: "cancelBatch"
  key_links:
...
4. Add corresponding TanStack Query mutation hooks: `useCancelBatchMutation`, `useRetryImageMutation`. Add a `useResultsQuery` using `useQuery` that calls `fetchResults(batchName)` with `enabled: !!batchName`.
  </action>
  <verify>
